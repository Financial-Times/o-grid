...
(function() {
	...
	var startBookmarklet = function($) {
		...
		var gdIsBuilt = false;
		var buildGridDisplayer = function(gridFramework) {

			var $gdContainer = $('#grid-displayer .gd-container');
			var $gdRow       = $('#grid-displayer .gd-row');
			var $gdTools     = $('#grid-displayer-tools');
			var colsHtml     = '';
			var gridNbcols   = parseInt($gdTools.find('#gdt-nbcols').val());
		...
		// Setters
		var setGridColor = function(gridColor, gutterless) {
		...
		var setGridGutters = function(gridGutterwidth) {
			var borderWidth = parseInt(gridGutterwidth.replace( /^\D+/g, '')) / 2;
			var unit = gridGutterwidth.substr(gridGutterwidth.length - 2, 2);
		...
		if ($('#grid-displayer').length) { // Close grid displayer when the bookmarklet is clicked for a second time
			...
			var gridToolsHtml = '<div id="grid-displayer-tools">';
